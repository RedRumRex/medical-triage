...
API key is read from the HF_TOKEN environment variable.
"""
...
def run_ai_baseline(task_id: str, model: str, seed: int) -> tuple[str, str]:
    ...
    # ✅ FIXED: read HF_TOKEN (competition variable) with OPENAI_API_KEY as fallback
    ...
    if not api_key:
        return (
            ...
            "Add it in your Space Settings → Variables and Secrets.",
            ""
        )
...
def step_env(req: StepRequest):
    ...
    action = TriageAction(rankings=[PatientRanking(**r) for r in req.rankings])
    _, reward, done, info = env.step(action)
    return {"reward": reward.total, "done": done, "info": info}
...
with gr.Blocks(title="Medical Triage Prioritization — OpenEnv", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Accordion("🤖 AI Baseline", open=False):
        key_status = "✅ `HF_TOKEN` detected in environment." \
            if os.environ.get("HF_TOKEN") else \
            "⚠️ `HF_TOKEN` not set. Add it in Space Settings → Secrets."
        gr.Markdown(
            f"{key_status}\n\n"

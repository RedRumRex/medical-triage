"""
inference.py — Medical Triage Prioritization
===================================
...
# ── LLM call ──────────────────────────────────────────────────────────────────
def get_triage_action(client: OpenAI, obs) -> tuple:
    ...
    try:
        ...
        return action, f"fallback_triage({len(obs.patients)}patients)", str(exc).replace("\n", " ")
...
def run_task(client: OpenAI, task_id: str) -> None:
    ...
    try:
        ...
        action, action_str, parse_error = get_triage_action(client, obs)

        try:
            _, reward_obj, done, info = env.step(action)
            reward    = round(reward_obj.total, 2)
            done_flag = done
            error     = parse_error  # carry parse error if any
        ...
        log_step(step=step, action=action_str, reward=reward, done=done_flag, error=error)
